...
- Is there "one true value" for the material properties of Aluminum?
  - No, even for a given thickness of an alloy at a specific angle, the `E` and `mu` had variations among observations.  
- How many aluminum alloys are in this dataset? How do you know?
  - There is only one alloy in the dataset. The value for column `alloy` doesn't change.
...
```{r q3-task, message=FALSE, warning=FALSE}
plot_E <-
  ggplot() +
  geom_point(
    data = df_stang_long,
...
- It's tempting to say that `E` and `mu` decreases as thickness increases. However, the relationship from the plot seems nonlinear and varying according to the angle and the thickness.
...
- Does this graph support or contradict the claim above?
  - The graph above shows some degree of variance in E, especially when the thickness is 0.081 versus any other. But I'm uncertain whether the degree of decrease in E is sufficient to claim that the elasticity is not an intensive property of the material since I don't know the how significant of a difference this is relative to other alloys. 
