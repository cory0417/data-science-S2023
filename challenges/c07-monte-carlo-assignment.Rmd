...
- Was your sample size $n$ large enough? Why do you say that?
  - I chose my sample size to be 1e7 because smaller sample sizes led to pretty big margin of error for high confidence levels. I think the sample size was large enough because the `lo` and `high` were both very close (both less than 0.002 away from pi) to the true value of pi even at 99% confidence level. However, it's worth noting that the calculated value of pi from this experiment was still `3.142074` which is accurate up to 3 significant figures.
