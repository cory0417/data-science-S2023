...
- Although the price increases as the carat increases generally, it’s
  not necessary that a diamond of higher carat will be more expensive
  than one of lower carat.
...
## Your code here!
diamonds %>%
  ggplot(aes(x = carat, y = price, color = cut), size = 1) +
  geom_point()
```
...
- The cut quality as well as the carat size make the diamonds more
  expensive.
- The cheaper diamonds of the same carat size are labeled generally
  ‘Fair’ and ‘Good’.
