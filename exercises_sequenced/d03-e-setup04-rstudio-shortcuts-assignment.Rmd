...
* Within the script editor or a chunk
    ...
    * `Shift` + `Cmd/Ctrl` + `M`    Insert '%>%'


    * `Cmd/Ctrl` + `Enter`    Run current line/selection


    * `F1` (Note: on a Mac you need to press `fn` + `F1`)   Show help for a function at cursor
...
* Within R Markdown
    ...
    * `Cmd/Ctrl` + `Alt` + `I`    Insert a code chunk


* Within a chunk

    * `Shift` + `Cmd/Ctrl` + `Enter`    Run the current chunk


    * `Ctrl` + `I` (`Cmd` + `I`)    Re-indent the line
...
Click the 'Outline' button below the 'Run' button or press Ctrl+Shift+O.
