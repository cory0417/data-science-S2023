...
- It mixed up the order of the rows and combined the data frames together. 
...
assertthat::assert_that(all.equal(df_q2, beatles_joined))
print("Nice!")
```
...
# TODO: Use expand_grid to create a criteria dataframe
  airports %>%
  filter(str_detect(name, "San Francisco | San Jose | Oakland", negate = FALSE)) %>% 
  select(dest = faa) %>% 
  expand_grid(month = 8:9)
