...
```
- They both do the same job but using the pipe operator gives more flexibility in further processing.
...
```
- The result with 'group_by' renders the 'price' column specific to each 'color' and 'clarity'.
...
'cov_depth' is much smaller than 'cov_diff'. 

- Why is `cov_diff` so large?
The difference between the depth and the computed depth is very small and dividing the standard division with it makes the 'cov_diff' very big. 
...
- The cuts don't necessary dictate the price of the diamonds -- I would have expected ideal cut to be generally the most expensive.
- The coefficient of variance is very large for all of them which shows that the cuts don't have a strong correlation with the prices. 
