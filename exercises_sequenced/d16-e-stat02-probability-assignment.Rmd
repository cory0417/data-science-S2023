...
df_z %>% 
  mutate(in_A = (z <= 0)) %>% 
  summarize(count_toal = n(), count_A = sum(in_A), fr = mean(in_A))
```

**Observations**:

-   What value of `fr` did you predict? 
My prediction of `fr` was 0.5.
...
-   What value does `fr` appear to be limiting towards? It appears to be limiting towards 0.95. Now that we know what probability is; let's connect the idea to *distributions*.
...
Note that, by definition, the CDF gives the probability over the set $A = {x' | x' <= x}$. Thus the CDF returns a probability (which explains the `p` prefix for R functions).
...
`fr` prediction was 0.5 and `p0` is 0.5.
