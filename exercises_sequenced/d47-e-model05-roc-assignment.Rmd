...
For instance in the heart disease case, we might be more concerned with flagging all possible cases of heart disease, particularly if follow-up examination can diagnose heart disease with greater precision. In that case, we might want to avoid false negatives but accept more false positives.
...
It's not a great classifier since the line stays close to the diagonal. It's sometimes even worse than random guessing. 
...
It's an effective classifier since it has the greates possible area under the curve. 

- What explains this model's performance? Is this model valid for prediction?
It's the predicting the heart disease using the result itself since `heart_disease = num > 0`.
...
It's more accurate since the area under the curve is greater than that of `fit_baseline`.
...
TPR increases as FPR increases. However, after a certain FPR, the increase in TPR starts to flatten. The “tradeoff” is that we can have an arbitrarily high TPR, but we “pay” for this through an increase in the FPR.
...
From the plot, a reasonable value for TPR would be about 0.9 since values any higher will require a significant drop in the probability threshold. Also, we need to maximize the True Positive Rate because it's crucial for patients to get treated. 

- What values of `pr_threshold` meet or exceed that TPR?
About 0.45. 

- What specific value of `pr_threshold` do you pick, and why?
I picked 0.45 since it increases TPR by a big amount while covering the desired TPR value. 
