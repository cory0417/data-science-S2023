...
I expected the prices to increase for better cuts.

- Did the model predictions match your expectations?
It matched only for upper carat diamonds. 
...
The lower model shows a decreasing relationship whereas the upper model shows a generally flat relationship. 

- How might this trend relate to the `price ~ cut` behavior we saw above?
For lower model, if the cut quality is lower, the carat size tends to be bigger, which might explain the higher price for lower cut qualities. 
...
It's a decreasing relationship. 
...
- What might account for this behavior?
  ...
  For the `cut_only` model, the cuts act as a surrogate for carat. However, when the carat is included in the model, it can uses carat to predict the price and can see the behavior of the cut more clearly.
