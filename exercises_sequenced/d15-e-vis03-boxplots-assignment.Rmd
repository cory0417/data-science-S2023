...
```{r q1-task}
diamonds %>%
  mutate(carat_cut = cut_width(x = carat, width = 0.5)) %>% 
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()
```

**Observations**
- The lower outliers/whiskers increases as the carat increases. However, the higher outliers/whiskers seems to be almost constant after an initial jump to >0.75 carat.
- The median price increases up to about 2 carat but stabilizes afterwards. 
...
Yes, the most common group changed from `Ideal`, `G` to `Premium`, `I`.
