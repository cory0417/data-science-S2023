...
- Both; since the physical molecular composition of the material could vary in the manufacturing process, the elasticity could vary as well. However, there is also a chance of induced variability from the experiment procedures. 
...
- real or induced?
- Both; as in the previous question, the molecular compositions, the total mass, the shape could cause real variability but also the testing process could cause induced variability. 
...
- Which is larger: The variability due to noise, or the variability due to deviation?
  - Variability due to deviation is larger. There is considerably more scatter between the red dots (each sample’s measurement mean), than there is scatter around each red dot.
...
- No, since some of the deviations show much greater strength, it would bias the mean towards a greater strength value.
...
- How does `Y_lo` compare with `Y_mean`?
  - It's a much smaller value than `Y_mean`. 
- Would the 10% quantile `Y_lo` be a conservative material strength for design?
  - It would be a conservative material strength for design.
...
- Is the new value `Y_lo_improved` more or less conservative than the original `Y_lo`?
  - The new value is less conservative than the original value. 
- What features about the data collection / dataset `df_meas` enable the calculation of `Y_lo_improved`?
  - There are multiple observations for the same `id_sample`.
- Measuring the strength of a part is a *destructive* experiment. Would it be possible to gather a dataset like `df_meas` in reality?
  - It might be challenging to use so much material to conduct multiple observations. 
- For what real properties (material, or otherwise) would it be possible to collect data like `df_meas`?
  - Any non-destructive testing, like mass of a part.
