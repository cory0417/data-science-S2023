...
No, they do not fully capture the trend.
...
fit_over <- 
  df_measurements %>% 
  lm(formula = y_meas ~ poly(x, degree = 15))
...
The predictions are very accurate at the measured points.

- How well do the predictions hold *outside* the measured points?
They are not at all accurate. 
...
`fit_over` has much lower MSE on `df_measurements`.

- Based *solely* on these results, which hyperparameter value (polynomial order) would you choose?
I would choose the higher order for hyperparameter. 

- Would that be a good choice? Why or why not?
It wouldn't be a good choice because the model is only applicable to the training dataset. 
...
I would pick the order 4. 

- Which selection leads to the correct choice?
The method on the validation dataset. 
...
The 4C's: carat, cut, clarity, and color. Depth was also a great predictor of price.

- Why do you think those predictors are informative?
In the EDA process, those were the 4 main predictors of the price.

- Why might the uninformative predictors not be helpful?
x, y, z were not very helpful together. 
...
They barely have any overlapping prices. 

- What effect would it have to train on `diamonds_test_bad` and then predict on `diamonds_validate_bad`?
It would be trying to predict more expensive diamonds based on trends of cheaper diamonds. 
