...
df_sample %>%
  summarize(
    ...
    lo = mean(price) - q95 * sd(price) / sqrt(n()),
    hi = mean(price) + q95 * sd(price) / sqrt(n()),
    price_threshold
  )
```
...
df_population_resample <- 
  bootstraps(df_sample, times = 1000) %>% 
  mutate(estimates = map(splits, fit_func))

int_pctl(df_population_resample, estimates, alpha = 0.01)
...
- Can you reject the null hypothesis **H_0**?
Yes, the the estimate is greater than 50%. 

- How does this affect your decision?
The decision will now be to buy the diamonds.
...
No, the hypothesis was not rejected. 

- What is the actual truth?
The mean price is lower than the threshold value.

- What decision did you make?
I decided not to buy the diamonds. 
...
proportion <- 
  df_population %>% 
  summarize(proportion = mean(cut == 'Ideal' | cut == 'Premium')) %>% 
  pull(proportion)
