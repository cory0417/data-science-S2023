...
It would probably increase as the price increases.
...
```{r q3-task}
gapminder %>% 
  mutate(continent = fct_relevel(as.factor(continent), c("Asia", "Europe", "Africa", "Oceania", "Americas"))) %>% 
  pull(continent) %>% 
  levels()
...
The distribution of `cty` is random and is hard to recognize.

*Sorted*
- Modify the code to sort `manufacturer`, then note any additional observations you make.
The distribution of `cty` is in an ascending order. 
