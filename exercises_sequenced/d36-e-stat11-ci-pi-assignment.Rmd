...
mu <- mean(diamonds_train$price)
sd <- sd(diamonds_train$price)
n <- nrow(diamonds_train) 
df_q2 <- 
  tibble(
    ci_lwr = mu - 1.96 * sd / sqrt(n),
    ci_upr = mu + 1.96 * sd / sqrt(n),
...
It's working to an extent since we assumed that it would be a normal distribution even though it's clearly not. CI seems quite close to the mean, PI theoretical should be able to cover 95% of all the prices. 

- What *assumptions* did you make when defining your CI and PI above? Are those assumptions valid for these data?
We assumed normal distribution with the sd and mean as that of the training dataset.
...
Yes, it does include the mean price.

  - Do we expect the CI to *always* include the true parameter?
  No we can't expect that. 
  
- Does your PI include the appropriate fraction of diamond prices?
It includes about 93.5% which is pretty close to 95%. 
