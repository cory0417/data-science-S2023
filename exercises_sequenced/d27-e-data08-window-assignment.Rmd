...
```{r ex-lead-lag}
## NOTE: No need to edi[t this; just an example
v <- c(1, 2, 3, 4, 5)
lead(v)
lag(v)
```
...
### __q1__ Use a window function to modify the following visual to color each segment differently based on whether the period of time was increasing or decreasing.
...
economics %>%
  ...
  geom_segment(
    data = . %>% mutate(trend = lead(unemploy, order_by = date) > unemploy),
    aes(
