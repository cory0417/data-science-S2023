...
`carat` is a continuous variable where as `cut` is a categorical variable. 

- Can we "multiply" `cut` by a constant? Why or why not?
We can't multiply it by a constant since it's not a number.

- Describe the difference between the `term`s for `carat` and `cut`.
There's a single term for `carat` but 4 for `cut`.

- How many `cut` terms does the model have? How many levels does `cut` have?
There are 4 terms for `cut` and `cut` has 5 levels (Fair, Good, Very Good, Premium, Ideal).
...
The fit from q1 returns 4 points for different cut status. So for a price vs carat plot, it's visually ineffective. 
...
My model is slightly more accurate than `fit_carat` since the MSE and the R-squared values are smaller and bigger respectively. 
...
df_train %>%
  ...
  geom_abline(slope = 1, intercept = 0, color = "grey50", linewidth = 2) +
  geom_point()
```
...
It's more accurate than the previous models.
...
The helper function `add_uncertainties()` added the columns `pred_fit` (the predicted price), as well as two new columns: `pred_lwr` and `pred_upr`. These are the bounds of a *prediction interval* (PI), an interval meant to capture *not* a future sample statistic, *but rather* a future observation.
